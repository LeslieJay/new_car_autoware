...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	: Node("yesense_publisher")
	{
		...
		*((unsigned int *)&yis_out) = 0u;		
		...
		// }
		driver_type = 1;
		RCLCPP_INFO(this->get_logger(), "第一次打印driver type%d\n", driver_type);
		// using namespace std;
		// int num;
		// cout << "请输入一个整数: ";
		// cin >> num;  // 程序会暂停，等待用户输入一个整数
		// =================================================
	...
	void timer_callback()
	{
		// RCLCPP_INFO(this->get_logger(), "driver type: %d", static_cast<int>(driver_type));	
		// RCLCPP_INFO(this->get_logger(), "第二次打印driver type%d\n", driver_type);
		...
		try {
			...
			// 1. 串口读取数据
			if (serial_drv_ros == driver_type) {
				// RCLCPP_INFO(this->get_logger(), "进入ser.available()");    
				if (ser.available()) {
					const size_t MAX_BUFFER_SIZE = 128;  // 确保缓冲区足够大
					bytes_read_r_buffer = std::min(ser.available(), MAX_BUFFER_SIZE);
					bytes_read_r_buffer = ser.read(r_buffer, bytes_read_r_buffer);
				}
				// RCLCPP_INFO(this->get_logger(), "离开ser.available()");    
			}
			else if (serial_drv_linux == driver_type) {
				bytes_read_r_buffer = read(fd, r_buffer, UART_RX_BUF_LEN);
				if (bytes_read_r_buffer == static_cast<size_t>(-1)) {
					RCLCPP_ERROR(this->get_logger(), "Serial read failed: %s", strerror(errno));
					return;
				}
			}
			else {
				RCLCPP_WARN(this->get_logger(), "Unknown driver type: %d", static_cast<int>(driver_type));
				return;
			}
				...
				if (ret == analysis_ok) {
					if (yis_out.content.valid_flg) {
						...
						try {
							...
							RCLCPP_ERROR(this->get_logger(), "Exception in publish_msg: %s", e.what());
						} catch (...) {
							RCLCPP_ERROR(this->get_logger(), "Unknown exception in publish_msg.");
						}
					}
				}
				else {
					RCLCPP_DEBUG(this->get_logger(), "data_proc returned error code: %d", ret);
				}
			} catch (const std::exception& e) {
				RCLCPP_ERROR(this->get_logger(), "Exception in data_proc: %s", e.what());
				return;
			}
			...
			RCLCPP_ERROR(this->get_logger(), "Unknown non-standard exception.");
		}
	}
	// void timer_callback()
	// {
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
    ...
    pub_imu_ros->publish(imu_ros_data);
  	...
  	// ================ update pose =================
	// frame_id = "tamagawa/imu_link";
    pose_data.header.frame_id 	= "tamagawa/imu_link";
    ...
    // ============ update imu marker ===============
    marker_info.header.frame_id = "tamagawa/imu_link";
