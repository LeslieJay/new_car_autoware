  ...
  is_freespace_parking_cb_running_{false}
{
  ...
  fixed_goal_planner_ = std::make_unique<DefaultFixedGoalPlanner>(parameters_);
...
bool GoalPlannerModule::isExecutionRequested() const
{
  ...
  if (!utils::isAllowedGoalModification(route_handler)) {
    ...
    return self_to_goal_arc_length >= 0.0 &&
           self_to_goal_arc_length <= parameters_.fixed_goal.request_length;
  }
