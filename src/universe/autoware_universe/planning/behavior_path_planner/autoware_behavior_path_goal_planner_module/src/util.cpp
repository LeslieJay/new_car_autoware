...
PathWithLaneId applyPreciseFixedGoalStop(
  const PathWithLaneId & path, const Pose & goal_pose,
  const GoalPlannerParameters::FixedGoal & fixed_goal_params)
{
  if (path.points.size() < 2) {
    return path;
  }

  if (fixed_goal_params.goal_resample_interval <= 0.0) {
    return path;
  }
  ...
  constexpr double default_resample_range = 3.0;
  const double resample_range =
    std::max(default_resample_range, fixed_goal_params.stop_margin + 2.0);
  const double start_s = std::max(0.0, total_s - resample_range);

  auto output = utils::resamplePathWithSpline(
    path, fixed_goal_params.goal_resample_interval, false, {start_s, total_s});

  if (output.points.empty()) {
    return path;
  }

  output.points.back().point.pose = goal_pose;
  output.points.back().point.longitudinal_velocity_mps = 0.0;

  if (fixed_goal_params.insert_stop_point_at_goal) {
    const size_t goal_idx = output.points.size() - 1;
    for (size_t i = 0; i < output.points.size(); ++i) {
      const double dist_to_goal =
        autoware::motion_utils::calcSignedArcLength(output.points, i, goal_idx);
      if (dist_to_goal <= fixed_goal_params.stop_margin + 1e-3) {
