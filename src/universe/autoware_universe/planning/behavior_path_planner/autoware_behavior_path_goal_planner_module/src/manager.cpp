...
std::unique_ptr<SceneModuleInterface> GoalPlannerModuleManager::createNewSceneModuleInstance()
{
  return std::make_unique<GoalPlannerModule>(
    name_, *node_, parameters_, lane_parking_timer_cb_group_, freespace_parking_timer_cb_group_,
  ...
  rclcpp::Node * node, const std::string & base_ns)
{
  ...
  // fixed goal
  {
    const std::string ns = base_ns + "fixed_goal.";
    p.fixed_goal.request_length = node->declare_parameter<double>(ns + "request_length");
    p.fixed_goal.goal_resample_interval =
      node->declare_parameter<double>(ns + "goal_resample_interval");
    p.fixed_goal.insert_stop_point_at_goal =
      node->declare_parameter<bool>(ns + "insert_stop_point_at_goal");
    p.fixed_goal.stop_margin = node->declare_parameter<double>(ns + "stop_margin");
  }
  ...
  [[maybe_unused]] const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  {
    const std::string ns = base_ns + "fixed_goal.";
    update_param<double>(parameters, ns + "request_length", p->fixed_goal.request_length);
    update_param<double>(
      parameters, ns + "goal_resample_interval", p->fixed_goal.goal_resample_interval);
    update_param<bool>(
      parameters, ns + "insert_stop_point_at_goal", p->fixed_goal.insert_stop_point_at_goal);
    update_param<double>(parameters, ns + "stop_margin", p->fixed_goal.stop_margin);
  }
  // goal search
