...
#include "stopper/stopper_gnss.hpp"
  ...
  logger_configure_(std::make_unique<autoware_utils_logging::LoggerLevelConfigure>(this))
{
  ...
  if (is_running(PoseEstimatorType::gnss)) {
    stoppers_.emplace(
      PoseEstimatorType::gnss, std::make_shared<stopper::StopperGnss>(this, shared_data_));
    sub_gnss_output_ = create_subscription<PoseCovStamped>(
      "~/input/gnss/pose_with_covariance", 5, /* this is not sensor topic */
      shared_data_->gnss_output_pose_cov.create_callback());
  }
