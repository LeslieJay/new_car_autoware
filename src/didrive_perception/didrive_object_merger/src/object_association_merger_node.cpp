...
#include "didrive/object_merger/object_association_merger_node.hpp"
...
bool isUnknownObjectOverlapped(
  ...
  // return autoware::object_recognition_utils::get2dOverlap(
  //     unknown_object, known_object, precision_threshold, recall_threshold,
  //     generalized_iou_threshold);
  const auto precision = autoware::object_recognition_utils::get2dPrecision(
      unknown_object, known_object);
  const auto recall = autoware::object_recognition_utils::get2dRecall(
      unknown_object, known_object);
  const auto generalized_iou =
      autoware::object_recognition_utils::get2dGeneralizedIoU(unknown_object,
                                                              known_object);
...
namespace {
std::map<int, double> convertListToClassMap(
    const std::vector<double>& distance_threshold_list) {
  std::map<int /*class label*/, double /*distance_threshold*/>
      distance_threshold_map;
  int class_label = 0;
  for (const auto& distance_threshold : distance_threshold_list) {
    distance_threshold_map.insert(
...
namespace didrive::object_merger {
ObjectAssociationMergerNode::ObjectAssociationMergerNode(
  ...
  overlapped_judge_param_.generalized_iou_threshold = convertListToClassMap(
   ...
   */
  overlapped_judge_param_.distance_threshold_map = convertListToClassMap(
      declare_parameter<std::vector<double>>("distance_threshold_list"));

  const auto tmp =
      this->declare_parameter<std::vector<int64_t>>("can_assign_matrix");
  ...
  data_association_ =
      std::make_unique<didrive::object_merger::DataAssociation>(
          can_assign_matrix, max_dist_matrix, max_rad_matrix, min_iou_matrix);

  // Create publishers and subscribers
  using std::placeholders::_1;
  using std::placeholders::_2;
  sync_ptr_ = std::make_shared<Sync>(SyncPolicy(sync_queue_size_), object0_sub_,
                                     object1_sub_);
  ...
  processing_time_publisher_ =
      std::make_unique<autoware::universe_utils::DebugPublisher>(
...
void ObjectAssociationMergerNode::objectsCallback(
  ...
  autoware_perception_msgs::msg::DetectedObjects transformed_objects0,
      transformed_objects1;
  if (!autoware::object_recognition_utils::transformObjects(
          *input_objects0_msg, base_link_frame_id_, tf_buffer_,
          transformed_objects0) ||
      !autoware::object_recognition_utils::transformObjects(
          *input_objects1_msg, base_link_frame_id_, tf_buffer_,
          transformed_objects1)) {
    return;
  }
  // build output msg
  ...
  Eigen::MatrixXd score_matrix = data_association_->calcScoreMatrix(
      transformed_objects1, transformed_objects0);
  data_association_->assign(score_matrix, direct_assignment,
                            reverse_assignment);
  for (size_t object0_idx = 0; object0_idx < objects0.size(); ++object0_idx) {
    const auto& object0 = objects0.at(object0_idx);
    if (direct_assignment.find(object0_idx) !=
        direct_assignment.end()) {  // found and merge
  ...
  if (remove_overlapped_unknown_objects_) {
    std::vector<autoware_perception_msgs::msg::DetectedObject> unknown_objects,
        known_objects;
    ...
    for (const auto& object : output_msg.objects) {
      if (autoware::object_recognition_utils::getHighestProbLabel(
              object.classification) == Label::UNKNOWN) {
  ...
  published_time_publisher_->publish_if_subscribed(merged_object_pub_,
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    didrive::object_merger::ObjectAssociationMergerNode)
