...
namespace didrive::ros2_bevfusion_async
{
  ...
  void NVBEVFusionNodeAsync::cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr pc_msg)
  {
    ...
    for (std::size_t i = 0; i < num_cameras_; ++i)
    {
      ...
      if (temp_image_msgs_[i] != nullptr)
      {
        ...
        double time_diff = std::abs(lidar_stamp_ - cam_stamp);
        if (time_diff >= max_camera_lidar_delay_)
        {
        ...
        {
          camera_masks_[i] = 1.0f;
          RCLCPP_DEBUG(this->get_logger(), "Camera %lu aligned with LiDAR (time diff: %.6f s)", i, time_diff);
        }
      }
      else
      {
        camera_masks_[i] = 0.0f;
        RCLCPP_DEBUG(this->get_logger(), "Camera %lu has no image data, using black image", i);
      }
    }
      ...
      const sensor_msgs::msg::Image::ConstSharedPtr msg, std::size_t camera_id)
  {
    ...
    // }
    {
      ...
      RCLCPP_DEBUG(this->get_logger(), "Received image for camera %lu, size: %dx%d", 
                   camera_id, msg->width, msg->height);
    }
    ...
    RCLCPP_DEBUG(this->get_logger(), "Images available: %d/%lu", 
                 num_valid_images, image_msgs_.size());
  }
    ...
    for (size_t i = 0; i < num_cameras_; ++i)
    {
      ...
      if (camera_masks[i] > 0.0f && img_msg)
      {
        ...
        else
        {
          ...
          RCLCPP_DEBUG(this->get_logger(), "Camera %zu: Using REAL image data (size: %s)", 
                       i, img_msg->encoding.c_str());
        }
      }
      else
      {
        ...
        if (camera_masks[i] <= 0.0f && img_msg)
        {
          RCLCPP_DEBUG(this->get_logger(), "Camera %zu: Masked due to time mismatch, using black image", i);
        }
        else
        {
          RCLCPP_DEBUG(this->get_logger(), "Camera %zu: No image data available, using black image", i);
        }
      }
