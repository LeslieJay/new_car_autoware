...
void PatchWorkpp::estimateGround(Eigen::MatrixXf cloud_in) {
  ...
  std::chrono::duration<double, std::ratio<1, 1000>> t_flush = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t1_1 - t1);
  std::chrono::duration<double, std::ratio<1, 1000>> t_czm = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t2 - t1_1);
  std::chrono::duration<double, std::ratio<1, 1000>> t_sort, t_pca, t_gle, t_revert;
  ...
  for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
    ...
    for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
      for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
        ...
        std::chrono::steady_clock::time_point t_bef_sort = std::chrono::steady_clock::now();
        sort(zone[ring_idx][sector_idx].begin(), zone[ring_idx][sector_idx].end(), point_z_cmp);

        std::chrono::steady_clock::time_point t_aft_sort = std::chrono::steady_clock::now();
        t_sort = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t_aft_sort - t_bef_sort);
        // ---------------------------------------------------------------------------------- //


        std::chrono::steady_clock::time_point t_bef_pca = std::chrono::steady_clock::now();
        extract_piecewiseground(
            zone_idx, zone[ring_idx][sector_idx], regionwise_ground_, regionwise_nonground_);

        std::chrono::steady_clock::time_point t_aft_pca = std::chrono::steady_clock::now();
  	 t_pca = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t_aft_pca - t_bef_pca);
        centers_.push_back(PointXYZ(pc_mean_(0), pc_mean_(1), pc_mean_(2)));
        normals_.push_back(PointXYZ(normal_(0), normal_(1), normal_(2)));


        std::chrono::steady_clock::time_point t_bef_gle = std::chrono::steady_clock::now();
        ...
        */
        ...
        std::chrono::steady_clock::time_point t_aft_gle = std::chrono::steady_clock::now();
        t_gle = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t_aft_gle - t_bef_gle);
      }


      std::chrono::steady_clock::time_point t_bef_revert = std::chrono::steady_clock::now();
      if (!candidates.empty()) {
      ...
      std::chrono::steady_clock::time_point t_aft_revert = std::chrono::steady_clock::now();
      t_revert = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t_aft_revert - t_bef_revert);
      concentric_idx++;
    }
  }


  std::chrono::steady_clock::time_point t_bef_update = std::chrono::steady_clock::now();
  update_elevation_thr();
  update_flatness_thr();

  std::chrono::steady_clock::time_point t_aft_update = std::chrono::steady_clock::now();
  std::chrono::duration<double, std::ratio<1, 1000>> t_update = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(t_aft_update - t_bef_update);

  std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
  std::chrono::duration<double, std::ratio<1, 1000>> time_taken = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1000>>>(end - beg);
  time_taken_ = time_taken.count();
  if (params_.verbose) {
    cout << "Time taken : " << time_taken.count()
         << "(ms) ~ "
         << t_flush.count() << "(flush) + "
         << t_czm.count() << "(czm) + "
         << t_sort.count() << "(sort) + "
         << t_pca.count() << "(pca) + "
         << t_gle.count() << "(estimate)" << endl;
