  ...
  const autoware_perception_msgs::msg::DetectedObjects::ConstSharedPtr input_msg)
{
  ...
  if (!tf_buffer_.canTransform(lanelet_frame_id_, source_frame, 
    input_msg->header.stamp, rclcpp::Duration::from_seconds(0.1), &error_msg)) {
     RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Skipping frame: %s", error_msg.c_str());
     return;
  }
  // === 【修改代码结束】 ===
