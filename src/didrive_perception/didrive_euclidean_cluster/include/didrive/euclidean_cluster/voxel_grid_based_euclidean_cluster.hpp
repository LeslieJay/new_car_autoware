...
#include "didrive/euclidean_cluster/euclidean_cluster_interface.hpp"
...
class VoxelGridBasedEuclideanCluster : public EuclideanClusterInterface {
 public:
  ...
  VoxelGridBasedEuclideanCluster(bool use_height, int min_cluster_size,
                                 int max_cluster_size);
  VoxelGridBasedEuclideanCluster(bool use_height, int min_cluster_size,
                                 int max_cluster_size, float tolerance,
                                 float voxel_leaf_size,
                                 int min_points_number_per_voxel);
  bool cluster(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& pointcloud,
               std::vector<pcl::PointCloud<pcl::PointXYZ>>& clusters) override;
  bool cluster(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& pointcloud,
  ...
  void setVoxelLeafSize(float voxel_leaf_size) {
 ...
 private:
  bool estimate_shape(const pcl::PointCloud<pcl::PointXYZ>& cluster,
