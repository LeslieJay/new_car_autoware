...
{
  using didrive::occupancy_grid_map::fusion_policy::log_odds_fusion::logOddsFusion;
...
{
  using didrive::occupancy_grid_map::fusion_policy::dempster_shafer_fusion::dempsterShaferFusion;
  const double MARGIN = 0.03;
  const double OCCUPIED = 1.0 - MARGIN;
  const double FREE = 0.0 + MARGIN;
  const double UNKNOWN = 0.5;
