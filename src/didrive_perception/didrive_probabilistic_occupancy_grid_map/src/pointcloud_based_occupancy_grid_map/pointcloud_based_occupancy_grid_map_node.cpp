...
/// @brief 点云占据栅格图节点构造函数，初始化参数、订阅者、发布者以及 CUDA / 调试工具等资源。
/// @param node_options ROS2 节点选项
...
/// @brief 障碍物点云回调，当与原始点云时间戳一致时，触发占据栅格图更新处理。
/// @param input_obstacle_msg 障碍物点云消息
  ...
  const PointCloud2::ConstSharedPtr & input_obstacle_msg)
{
  ...
  if (obstacle_pointcloud_.header.stamp == raw_pointcloud_.header.stamp) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Timestamps not matched: obstacle_pointcloud_.header.stamp = %u, raw_pointcloud_.header.stamp = %u",
                obstacle_pointcloud_.header.stamp.nanosec,
                raw_pointcloud_.header.stamp.nanosec);
    // std::cout << "[didrive_occupancy_grid_map][obstaclePointcloudCallback] Timestamps not matched: obstacle_pointcloud_.header.stamp = "
    //           << obstacle_pointcloud_.header.stamp.nanosec
    //           << ", raw_pointcloud_.header.stamp = "
    //           << raw_pointcloud_.header.stamp.nanosec << std::endl;
  }
}

/// @brief 原始点云回调，当与障碍物点云时间戳一致时，触发占据栅格图更新处理。
/// @param input_raw_msg 原始点云消息
...
/// @brief 根据处理时间更新诊断信息，判断是否超出阈值并发布诊断状态。
/// @param processing_time_ms 单帧处理耗时（毫秒）
...
/// @brief 同时接收到障碍物点云和原始点云后的主处理流程，完成高度滤波、坐标变换、
///        占据栅格图更新以及结果发布。
void PointcloudBasedOccupancyGridMapNode::onPointcloudWithObstacleAndRaw()
{
  ...
  if (use_height_filter_) {
    ...
    if (raw_pointcloud_.header.frame_id != base_link_frame_) {
      if (!utils::transformPointcloudAsync(
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Failed to transform raw pointcloud to base_link frame");
        return;
      }
    }
    if (obstacle_pointcloud_.header.frame_id != base_link_frame_) {
      if (!utils::transformPointcloudAsync(
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Failed to transform obstacle pointcloud to base_link frame");
        return;
      }
    }
  ...
  RCLCPP_INFO(this->get_logger(),
              "Height filter applied: min_height = %f, max_height = %f",
  ...
  {  // create occupancy grid map and publish it
    RCLCPP_INFO(this->get_logger(),
                "Creating occupancy grid map and publishing it");
  ...
  if (enable_single_frame_mode_) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Updating and publishing occupancy grid map");
    std::unique_ptr<ScopedTimeTrack> inner_st_ptr;
    if (time_keeper_) {
      RCLCPP_INFO(this->get_logger(),
                "time_keeper_ is not null");
      inner_st_ptr =
  ...
  if (debug_publisher_ptr_ && stop_watch_ptr_) {
    ...
    const double pipeline_latency_ms =
      std::chrono::duration<double, std::milli>(
        std::chrono::nanoseconds((this->get_clock()->now() - raw_pointcloud_.header.stamp).nanoseconds())).count();
...
/// @brief 将内部 OccupancyGridMap/Costmap2D 表示转换为 ROS OccupancyGrid 消息指针。
/// @param frame_id     输出消息的坐标系 ID
/// @param stamp        时间戳
/// @param robot_pose_z 机器人在 Z 方向的位置（用于设置消息原点 z）
/// @param occupancy_grid_map 概率占据栅格图对象
/// @return 填充完成的 OccupancyGrid 智能指针
