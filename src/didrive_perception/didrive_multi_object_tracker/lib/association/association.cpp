...
#include "didrive/multi_object_tracker/association/association.hpp"
...
#include "didrive/multi_object_tracker/association/solver/gnn_solver.hpp"
#include "didrive/multi_object_tracker/utils/utils.hpp"
// #include "autoware/object_recognition_utils/object_recognition_utils.hpp"
// #include "autoware/universe_utils/geometry/geometry.hpp"
// #include "autoware/universe_utils/geometry/boost_polygon_utils.hpp"
#include "clipper2/clipper.h"
#include "fast_iou.hpp"
// #include <opencv2/opencv.hpp>

namespace
{
  double getMahalanobisDistance(const geometry_msgs::msg::Point &measurement,
                                ...
                                const Eigen::Matrix2d &covariance)
  {
    ...
    Eigen::MatrixXd mahalanobis_squared =
        (measurement_point - tracker_point).transpose() * covariance.inverse() *
        (measurement_point - tracker_point);
    return std::sqrt(mahalanobis_squared(0));
  }

  Eigen::Matrix2d getXYCovariance(
      const geometry_msgs::msg::PoseWithCovariance &pose_covariance)
  {
  ...
  double getFormedYawAngle(const geometry_msgs::msg::Quaternion &measurement_quat,
                           const geometry_msgs::msg::Quaternion &tracker_quat,
                           const bool distinguish_front_or_back = true)
  {
    ...
    const double tracker_yaw =
        autoware::universe_utils::normalizeRadian(tf2::getYaw(tracker_quat));
    ...
    // Fixed measurement_yaw to be in the range of +-90 or 180 degrees of
    // X_t(IDX::YAW)
  ...
  // Clipper2Lib::Path64 to_clipper_path(const autoware_utils_geometry::Polygon2d &poly, double scale = 1e6)
  // {
  //   Clipper2Lib::Path64 path;
  //   for (const auto &pt : poly.outer())
  //   {
  //     path.emplace_back(
  //         static_cast<int64_t>(boost::geometry::get<0>(pt) * scale),
  //         static_cast<int64_t>(boost::geometry::get<1>(pt) * scale));
  //   }
  //   return path;
  // }

  // Polygon to_polygon(const autoware_utils_geometry::Polygon2d &poly)
  // {
  //   Polygon polygon;
  //   for (const auto &pt : poly.outer())
  //   {
  //     Point pt_vec(boost::geometry::get<0>(pt),
  //                  boost::geometry::get<1>(pt));
  //     polygon.emplace_back(pt_vec);
  //   }
  //   return polygon;
  // }

  // double calculateIoUWithClipper2(
  //     const autoware_utils_geometry::Polygon2d &poly1,
  //     const autoware_utils_geometry::Polygon2d &poly2, double min_area = 1e-3)
  // {
  //   // Step 1: Convert your polygons to Clipper paths
  //   Clipper2Lib::Path64 subj = to_clipper_path(poly1);
  //   Clipper2Lib::Path64 clip = to_clipper_path(poly2);

  //   // 注意：这里必须使用 Paths64（vector of Path64）
  //   Clipper2Lib::Paths64 subj_paths = {subj};
  //   Clipper2Lib::Paths64 clip_paths = {clip};

  //   // Step 2: Compute intersection
  //   Clipper2Lib::Paths64 solution_intersection;
  //   {
  //     Clipper2Lib::Clipper64 c;
  //     c.AddSubject(subj_paths);
  //     c.AddClip(clip_paths);
  //     c.Execute(Clipper2Lib::ClipType::Intersection, Clipper2Lib::FillRule::NonZero, solution_intersection);
  //   }

  //   double area_intersection = 0.0;
  //   for (const auto &p : solution_intersection)
  //   {
  //     area_intersection += Clipper2Lib::Area(p);
  //   }

  //   // Step 3: Compute union
  //   Clipper2Lib::Paths64 solution_union;
  //   {
  //     Clipper2Lib::Clipper64 c;
  //     c.AddSubject(subj_paths);
  //     c.AddClip(clip_paths);
  //     c.Execute(Clipper2Lib::ClipType::Union, Clipper2Lib::FillRule::NonZero, solution_union);
  //   }

  //   double area_union = 0.0;
  //   for (const auto &p : solution_union)
  //   {
  //     area_union += Clipper2Lib::Area(p);
  //   }

  //   if (area_union < min_area)
  //   {
  //     return 0.0; // 防止除以零
  //   }
...
namespace didrive::multi_object_tracker
{
  ...
  DataAssociation::DataAssociation(std::vector<int> can_assign_vector,
                                   std::vector<double> max_dist_vector,
                                   std::vector<double> max_area_vector,
                                   std::vector<double> min_area_vector,
                                   std::vector<double> max_rad_vector,
                                   std::vector<double> min_iou_vector)
      : score_threshold_(0.01)
  {
    {
      const int assign_label_num =
          static_cast<int>(std::sqrt(can_assign_vector.size()));
    ...
    {
      const int max_dist_label_num =
          static_cast<int>(std::sqrt(max_dist_vector.size()));
    ...
    {
      const int max_area_label_num =
          static_cast<int>(std::sqrt(max_area_vector.size()));
    ...
    {
      const int min_area_label_num =
          static_cast<int>(std::sqrt(min_area_vector.size()));
    ...
    {
      const int max_rad_label_num =
          static_cast<int>(std::sqrt(max_rad_vector.size()));
    ...
    {
      const int min_iou_label_num =
          static_cast<int>(std::sqrt(min_iou_vector.size()));
  ...
  void DataAssociation::assign(const Eigen::MatrixXd &src,
                               ...
                               std::unordered_map<int, int> &reverse_assignment)
  {
    ...
    // Solve
    gnn_solver_ptr_->maximizeLinearAssignment(score, &direct_assignment,
                                              &reverse_assignment);
    ...
    for (auto itr = reverse_assignment.begin();
         itr != reverse_assignment.end();)
    {
      ...
      const std::list<std::shared_ptr<Tracker>> &trackers)
  {
         ...
         ++tracker_itr, ++tracker_idx)
    {
      ...
      for (size_t measurement_idx = 0;
           measurement_idx < measurements.objects.size(); ++measurement_idx)
      {
        ...
        if (can_assign_matrix_(tracker_label, measurement_label))
        {
          ...
          (*tracker_itr)
              ->getTrackedObject(measurements.header.stamp, tracked_object);
          ...
          if (passed_gate)
          {
            ...
            const double angle = getFormedYawAngle(
                measurement_object.kinematics.pose_with_covariance.pose
                    .orientation,
                tracked_object.kinematics.pose_with_covariance.pose.orientation,
                false);
            if (std::fabs(max_rad) < M_PI &&
                std::fabs(max_rad) < std::fabs(angle))
              passed_gate = false;
          }
          ...
          if (passed_gate)
          {
            ...
            const double area =
                autoware::universe_utils::getArea(measurement_object.shape);
          ...
          if (passed_gate)
          {
            const double min_iou =
                min_iou_matrix_(tracker_label, measurement_label);
            ...
            const auto source_polygon = autoware_utils_geometry::to_polygon2d(measurement_object);
            const auto target_polygon = autoware_utils_geometry::to_polygon2d(tracked_object);
            // 获取旋转矩形
            cv::RotatedRect rectA = utils::GetMinAreaRect(source_polygon);
            cv::RotatedRect rectB = utils::GetMinAreaRect(target_polygon);

            // 计算 IoU
            const double iou = utils::ComputeRotatedRectIoU(rectA, rectB);
