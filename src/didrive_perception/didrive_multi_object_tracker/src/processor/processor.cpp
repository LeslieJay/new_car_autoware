...
#include "didrive/multi_object_tracker/tracker/tracker.hpp"
...
void TrackerProcessor::removeOverlappedTracker(const rclcpp::Time & time)
{
  ...
  for (auto itr1 = list_tracker_.begin(); itr1 != list_tracker_.end(); ++itr1) {
    ...
    for (auto itr2 = std::next(itr1); itr2 != list_tracker_.end(); ++itr2) {
      ...
      const auto source_polygon = autoware_utils_geometry::to_polygon2d(object1);
      const auto target_polygon = autoware_utils_geometry::to_polygon2d(object2);
                // 获取旋转矩形
