#!/usr/bin/env python3
"""Launch Autoware, BYD CAN drivers, lidar, auto engage, and agv_to_rcs together."""
...
def _launch_everything(context: LaunchContext):
    ...
    # ===== 新增：使用 Node 启动 agv_to_rcs 的 main 节点 =====
    agv_to_rcs_main_node = Node(
        package="agv_to_rcs",
        executable="main",
        name="agv_to_rcs_main",
        output="both",
        emulate_tty=True,
        arguments=["--ros-args", "--log-level", log_level],
    )

    # ===== 新增：使用 Node 启动 agv_to_rcs 的 autoware_pose_to_rcs_pose 节点 =====
    agv_pose_to_rcs_node = Node(
        package="agv_to_rcs",
    ...
    if not as_bool(context, "enable_pedestrian_safety_stop", default=True):
        ...
        actions.extend([safety_transition, safety, wait_safety])

    # 最后统一添加两个 agv_to_rcs 节点
    actions.append(agv_to_rcs_main_node)
    actions.append(agv_pose_to_rcs_node)
