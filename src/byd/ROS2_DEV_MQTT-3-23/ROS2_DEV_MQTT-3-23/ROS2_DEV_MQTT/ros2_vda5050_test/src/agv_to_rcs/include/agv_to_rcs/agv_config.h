...
# include "ref_slam_interface/msg/vehicle_status.hpp"

using BatteryControl = ref_slam_interface::srv::BatteryControl;
using BatteryState   = ref_slam_interface::msg::BatteryState;
using VehicleStatus  = ref_slam_interface::msg::VehicleStatus;

using PoseType = geometry_msgs::msg::PoseWithCovarianceStamped;
#define AGV_CONFIG_LASER_VERSION  // 标识这是激光版本
...
struct OrderMessages{
    ...
    // 数据填充
    // 将所有节点下的action提取出来，存入msg_state的actionStates中，因为很多动作和点并不是绑定的，比如清洁从a开始，c结束，b点无动作，比如到a之前就要起升货叉，所有统一存放处理，和nodes相关但不强绑定，同时记录agv的运行状态，订单执行情况，是否有错误等等
