  ...
  canceled_{false}
{
  ...
  const auto qos = rclcpp::QoS(
    rclcpp::KeepLast(10)).reliable()           // 使用RELIABLE确保所有帧被接收
    .durability_volatile() // 不要求发布者保留历史消息
    .deadline(std::chrono::milliseconds(10)); // 设置截止时间，避免处理过时帧

  use_image_transport_ = declare_parameter("use_image_transport", true);
  image_topic_ = declare_parameter("image_topic", "image_raw");

  if (use_image_transport_) {
    camera_transport_pub_ = image_transport::create_camera_publisher(this, image_topic_,
                                                                    qos.get_rmw_qos_profile());
  } else {
    image_pub_ = create_publisher<sensor_msgs::msg::Image>(image_topic_, qos);
    info_pub_ = create_publisher<sensor_msgs::msg::CameraInfo>(image_topic_ + "/camera_info", qos);
  }
  ...
  undistort_enabled_ = declare_parameter("undistort_enabled", false);
  if (undistort_enabled_) {
    RCLCPP_INFO(get_logger(), "%s Open undistort ...", image_topic_.c_str());
  }
  ...
  capture_thread_ = std::thread{
    [this]() -> void {
      ...
      auto frame_interval = std::chrono::duration_cast<clock_type::duration>(
          std::chrono::duration<double>(1.0 / target_frequency)
      );
      auto next_capture_time = clock_type::now();

      // Start capture loop
      while (rclcpp::ok() && !canceled_.load()) {
        // 等待直到下一帧时间点
        std::this_thread::sleep_until(next_capture_time);
        next_capture_time += frame_interval;  // 更新下一帧时间点

        // auto time_since_epoch = next_capture_time.time_since_epoch();
        // auto millis_since_epoch = std::chrono::duration_cast<std::chrono::milliseconds>(
        //             time_since_epoch).count();
        // RCLCPP_INFO(get_logger(), "Capture next_capture_time: %f ms, target_frequency: %f", static_cast<double>(millis_since_epoch), static_cast<double>(target_frequency));
        ...
        // if (undistort_enabled_) {
        //   img->width = INFER_IMG_WIDTH;
        //   img->height = INFER_IMG_HEIGHT;
        // }
...
V4L2Camera::~V4L2Camera()
{
  ...
  cudaErrorCheck(cudaStreamDestroy(stream_));
}
...
#ifdef ENABLE_CUDA
  // init undistorters
  std::string device = get_parameter("video_device").as_string();
  is_camera_30_ = declare_parameter<int>("fov") == 30;
  if (image_size[0] == 3840 && image_size[1] == 2160) {
    is_params_800w_ = true;
  }
  else {
    is_params_800w_ = false;
  }
  RCLCPP_INFO(get_logger(), "is_params_800w_: %d, is_camera_30_: %d", is_params_800w_, is_camera_30_);
  
  // create streams
  cudaErrorCheck(cudaStreamCreate(&stream_));
  nppGetStreamContext(&nppStreamCtx_);
  nppStreamCtx_.hStream = stream_;
  nppStreamCtx_.nCudaDeviceId = 0;
  init_undistorters();
...
void V4L2Camera::init_undistorters() {
  // 计算映射矩阵
  cv::Mat mapX_ft, mapY_ft, mapX_800w, mapY_800w, mapX_300w, mapY_300w;
  cv::initUndistortRectifyMap(
      cameraMatrix_ft, distCoeffs_ft, cv::Mat(), new_cameraMatrix_ft,
      cv::Size(IMG_WIDTH_200W, IMG_HEIGHT_200W), CV_32FC1, mapX_ft, mapY_ft);
  cv::initUndistortRectifyMap(
      cameraMatrix_fw, distCoeffs_fw, cv::Mat(), new_cameraMatrix_fw,
      cv::Size(IMG_WIDTH_200W, IMG_HEIGHT_200W), CV_32FC1, mapX_800w, mapY_800w);
  cv::initUndistortRectifyMap(cameraMatrix_lf, distCoeffs_lf, cv::Mat(),
                              new_cameraMatrix_lf,
                              cv::Size(IMG_WIDTH_200W, IMG_WIDTH_200W),
                              CV_32FC1, mapX_300w, mapY_300w);
  // 初始化参数
  params_800w_.src_img_width = IMG_WIDTH_800W;
  params_800w_.src_img_height = IMG_HEIGHT_800W;
  params_800w_.src_img_size = params_800w_.src_img_width *
                              params_800w_.src_img_height * channels_ *
                              sizeof(unsigned char);
  params_800w_.dst_img_width = IMG_WIDTH_200W;
  params_800w_.dst_img_height = IMG_HEIGHT_200W;
  params_800w_.dst_img_size = params_800w_.dst_img_width *
                              params_800w_.dst_img_height * channels_ *
                              sizeof(unsigned char);

  params_300w_.src_img_width = IMG_WIDTH_300W;
  params_300w_.src_img_height = IMG_HEIGHT_300W;
  params_300w_.src_img_size = params_300w_.src_img_width *
                              params_300w_.src_img_height * channels_ *
                              sizeof(unsigned char);
  params_300w_.dst_img_width = IMG_WIDTH_200W;
  params_300w_.dst_img_height = IMG_HEIGHT_200W;
  params_300w_.dst_img_size = params_300w_.dst_img_width *
                              params_300w_.dst_img_height * channels_ *
                              sizeof(unsigned char);
  
  // 畸变参数数据拷贝到 GPU
  if (is_params_800w_) {
      // 申请显存
      map_size_ = IMG_WIDTH_200W * IMG_HEIGHT_200W * sizeof(float);
      img_size_ = IMG_WIDTH_200W * IMG_HEIGHT_200W * channels_ * sizeof(unsigned char);
      cudaErrorCheck(cudaMalloc(&d_dst_, img_size_));
      cudaErrorCheck(cudaMalloc(&d_mapX_, map_size_));
      cudaErrorCheck(cudaMalloc(&d_mapY_, map_size_));
      cudaErrorCheck(cudaMemcpy(d_mapX_, (float *)mapX_800w.data, map_size_,
                            cudaMemcpyHostToDevice));
      cudaErrorCheck(cudaMemcpy(d_mapY_, (float *)mapY_800w.data, map_size_,
                              cudaMemcpyHostToDevice));
  }
  else {
    // 申请显存
    ...
    cudaErrorCheck(cudaMalloc(&d_mapY_, map_size_));
    if (is_camera_30_) {
      cudaErrorCheck(cudaMemcpy(d_mapX_, (float *)mapX_ft.data, map_size_,
                            cudaMemcpyHostToDevice));
      cudaErrorCheck(cudaMemcpy(d_mapY_, (float *)mapY_ft.data, map_size_,
                              cudaMemcpyHostToDevice));
    }
    else {
      cudaErrorCheck(cudaMemcpy(d_mapX_, (float *)mapX_300w.data, map_size_,
                              cudaMemcpyHostToDevice));
      cudaErrorCheck(cudaMemcpy(d_mapY_, (float *)mapY_300w.data, map_size_,
                              cudaMemcpyHostToDevice));
    }
  }
}

sensor_msgs::msg::Image::UniquePtr V4L2Camera::undistortOnGpu(sensor_msgs::msg::Image const & img)
{
  if ((img.encoding != sensor_msgs::image_encodings::YUV422 &&
       img.encoding != sensor_msgs::image_encodings::YUV422_YUY2) ||
      output_encoding_ != sensor_msgs::image_encodings::RGB8) {
  ...
  auto outImg = std::make_unique<sensor_msgs::msg::Image>();
  if (is_params_800w_) {
    outImg->width = IMG_WIDTH_200W;
    outImg->height = IMG_HEIGHT_200W;
    outImg->step = IMG_WIDTH_200W * 3;
    outImg->encoding = output_encoding_;
    outImg->data.resize(IMG_WIDTH_200W * IMG_HEIGHT_200W * 3);
  }
  else {
    outImg->width = IMG_WIDTH_200W;
    outImg->height = IMG_HEIGHT_200W;
  ...
  // 手动指定步长，YUV422每个像素2通道，RGB每个像素3通道
  size_t yuv_valid_step = img.width * 2;
  src_dev_->Allocate(img.width, img.height, yuv_valid_step, 2);
  size_t rgb_valid_step = img.width * 3;
  dst_dev_->Allocate(img.width, img.height, rgb_valid_step, 3);
  ...
  NppStatus res;
  if (img.encoding == sensor_msgs::image_encodings::YUV422_YUY2) {
    res = nppiYUV422ToRGB_8u_C2C3R_Ctx(src_dev_->dev_ptr,
                                             src_dev_->step_bytes,
                                             dst_dev_->dev_ptr,
                                             dst_dev_->step_bytes,
                                             roi,
    ...
    res = nppiCbYCr422ToRGB_8u_C2C3R_Ctx(src_dev_->dev_ptr,
                                     src_dev_->step_bytes,
                                     dst_dev_->dev_ptr,
                                     dst_dev_->step_bytes,
                                     roi,
  ...
  // rgb去畸变
  if (is_params_800w_) {
    distort_CUDA(params_800w_, d_mapX_, d_mapY_, dst_dev_->dev_ptr, d_dst_, stream_);
  }
  else {
    distort_CUDA(params_300w_, d_mapX_, d_mapY_, dst_dev_->dev_ptr, d_dst_, stream_);
  }

  // gpu -> ros cpu
  cudaErrorCheck(cudaMemcpyAsync(outImg->data.data(), d_dst_, img_size_,
                                  cudaMemcpyDeviceToHost, stream_));
  cudaErrorCheck(cudaStreamSynchronize(stream_));
  return outImg;
